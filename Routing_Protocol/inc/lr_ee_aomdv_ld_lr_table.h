...
public:
    ...
    void update_LoRa_path() //Used to update what path sould be chosen. For now forward to path with the smallest hop
    {
        ...
        //Set initial next hop
        if (m_route_list.size()>0)
        {
                    //std::cout<<"Size of known routes is "<<m_route_list.at(0).get_next_hop()<<endl;
                    m_LoRa_route.SetNextHop(m_route_list.at(0).get_next_hop());
            }
        for (int i = 1; i<m_route_list.size(); i++)
        {
            //std::cout<<"Current route came from "<<m_route_list.at(i).get_neighbour_source()<<" load is "<< m_route_list.at(i).get_load()<<" hop is "<<m_route_list.at(i).get_hopcount()<<endl;
...
///////////////////////////////////////////////
    ...
    {
        //std::cout<<"addRoutingEntity with next hop "<<add.get_next_hop()<<endl;
...
struct IsNeigh
{
   ...
   void SetRoute (LoRaRoute r)
   {
    ...
    std::vector<uint8_t> GetNextHop (uint8_t neighbour_address) const
    {
        ...
        //std::cout<<"Getting hop list for "<<neighbour_address<<endl;
