...
uint16_t s_tx_sent; //amount of packets sent
//Tracks to whom the messages where sent to
//Tracks the packets to be sent over for retrying
//KEY           VALUE
//neighbour     tx_packet
std::map<uint8_t, uint16_t> s_tx_neighbour; 

//Function that maintains the tx_neighbour tracker
void update_tx_neighbour(uint8_t add);

uint16_t s_rx_count; //keeps track of messages received per epoch count
//Tracks the packets to be sent over for retrying
//KEY           VALUE
//neighbour     rx_packet
std::map<uint8_t,uint16_t> s_neighbour_rx;
//Tracks the RSSI values of links
//KEY           VALUE
//neighbour     RSSI
std::map<uint8_t,int16_t> s_neighbour_RSSI;
//Tracks the SNR values of links
//KEY           VALUE
//neighbour     rx_packet
std::map<uint8_t,uint8_t> s_neighbour_SNR;
...
void update_packet_cetx(std::vector<uint8_t> packet, uint32_t cetx, uint8_t type)
{
