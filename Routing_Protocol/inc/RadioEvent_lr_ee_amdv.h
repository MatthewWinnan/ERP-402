...
void Simulation_Update_neighbour_rx(uint8_t neigh_address,uint8_t Snr,int16_t Rssi);
...
//For simulations
extern uint16_t s_rx_count; //keeps track of messages received per epoch count
extern std::map<uint8_t,uint16_t> s_neighbour_rx;
extern std::map<uint8_t,int16_t> s_neighbour_RSSI;
extern std::map<uint8_t,uint8_t> s_neighbour_SNR;
extern uint8_t device_ip_address ;
...
void add_rx_data_queue(LoRaMacEventInfo* data){
    ...
    if ((radio_data.at(2) != 20) && (radio_data.at(2) != 0) && (radio_data.at(1) == device_ip_address ))
    {
        Update_neighbour_rx(radio_data.at(2));//Updating receiving from neighbour
        Simulation_Update_neighbour_rx(radio_data.at(2),data->RxSnr,data->RxRssi);//Updating receiving from neighbour simulation
        }
    #elif ACTIVE_USER == CLIENT_NODE
    if ((radio_data.at(2) != 100) && (radio_data.at(2) != 0) && (radio_data.at(1) == device_ip_address ) )
    {
        Update_neighbour_rx(radio_data.at(2));//Updating receiving from neighbour
        Simulation_Update_neighbour_rx(radio_data.at(2),data->RxSnr,data->RxRssi);//Updating receiving from neighbour simulation
        }
    #else
    if ((radio_data.at(2) != 0) && (radio_data.at(1) == device_ip_address ) )
    {
    Update_neighbour_rx(radio_data.at(2));//Updating receiving from neighbour
    Simulation_Update_neighbour_rx(radio_data.at(2),data->RxSnr,data->RxRssi);//Updating receiving from neighbour simulation
    }
...
//Can at the same time increase tx amount since the same
    ...
    {
        //empty add a new entry
        s_neighbour_rx.insert(std::make_pair(neigh_address, 1));
    }
    else 
    {
        //Find if it is in the list
        auto entry = s_neighbour_rx.find(neigh_address);
        if (entry->first == neigh_address)
        {
            //entry found now update count
            entry->second++;
    ...
    {
        //empty add a new entry
        s_neighbour_RSSI.insert(std::make_pair(neigh_address, RSSI));
    }
    else 
    {
        //Find if it is in the list
        auto entry = s_neighbour_RSSI.find(neigh_address);
        if (entry->first == neigh_address)
        {
            //entry found now update count
    ...
    {
        //empty add a new entry
        s_neighbour_SNR.insert(std::make_pair(neigh_address, Snr));
    }
    else 
    {
        //Find if it is in the list
        auto entry = s_neighbour_SNR.find(neigh_address);
        if (entry->first == neigh_address)
        {
            //entry found now update count
